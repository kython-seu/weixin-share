 ...
 * @author Javen
 */
public class Users_back extends Model<Users_back> {

	private static final long serialVersionUID = 6204222383226990020L;
	
	static Log log = Log.getLog(Users_back.class);
	
	public static final Users_back me = new Users_back();
	...
	public boolean save(String openId,String nickName,String unionid,String headimgurl,String country,String city,String province ,int sex){
		 ...
		 *    如果不存在就保存
		 */
		Users_back user = findByOpenId(openId);
		if (user!=null) {
	 ...
	 */
	public Page<Users_back> paginate(int pageNumber, int pageSize) {
		return paginate(pageNumber, pageSize, "select *", "from users order by id asc");
	}
	
	public Users_back findByOpenId(String openId){
	 ...
	 * @return 
	 */
	public List<Users_back> findByMap(Map<String, Object> paras) {
